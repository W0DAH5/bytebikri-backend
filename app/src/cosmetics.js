...
// The MODEL (power ladder + motif catalog) lives in its own module, because both
// plus.js and this file read it and neither may import the other. Re-exported so
// the engine's public list stays one import — and the list of levels lives once.
import {
  POWER, POWER_KEYS, powerIndex, powerOf,
  MOTIFS, MOTIF_KEYS, MOTIF_KINDS, motifOf, motifsFor,
} from './cosmetic-model.js';
export { POWER, POWER_KEYS, powerIndex, powerOf, MOTIFS, MOTIF_KEYS, MOTIF_KINDS, motifOf, motifsFor };
...
export const SLOTS = [
  ...
  {
    // THE MOTIF — the identity axis (PREMIUM_COSMETICS.md §5). Power decides how
    // strong a treatment may be; the motif decides what it is. A gold member wearing
    // the Golden Lotus and one wearing the Golden Dragon carry the same treatment
    // and different identities.
    //
    // A PERSON slot: it decorates the person, travels with the person, and only
    // bytebikri renders it. A store may raise a person's power (their tier does) but
    // may not paint the person's name. `kind: 'icon'` is the picker control that
    // draws the actual drawing — a tile that IS the thing, like the ring's tile is
    // the ring — so the picker cannot promise a mark the product does not draw.
    //
    // `moves` follows the kind, and the kinds were chosen for it: a `motif` is an
    // 18 px mark beside the name and never moves (a moving 18 px icon is hostile to
    // the vestibular, and the mark has to be readable at the size it is read at);
    // a `mascot` breathes — resting pose, one idle, a hover reaction, never more,
    // and on one's own card rather than a roster row.
    key: 'motif',
    column: 'plus_motif',
    owner: 'person',
    grant: 'plus',
    kind: 'icon',
    label: 'Your mark',
    question: 'What wears beside your name.',
    hint: 'A mark is a personality, not a rank: the tier decides how strong it '
      + 'wears, and this decides what it is. A mark never moves; a mascot breathes '
      + 'on your own card only.',
    values: [
      {
        key: 'none',
        label: 'No mark',
        moves: false,
        words: 'Your name, as it is — no mark beside it, no mascot on your card.',
      },
      ...MOTIFS.map((m) => ({
        key: m.key,
        label: m.name,
        hint: m.treatment,
        moves: m.kind === 'mascot',
        // The picker's tile IS the drawing (like the ring's tile is the ring), and the
        // name's mark and the card's mascot reuse the same field, so the product cannot
        // promise a mark it does not have.
        asset: m.asset,
        words: m.kind === 'mascot'
          ? `${m.name} — a character on your card: ${m.treatment}`
          : `${m.name} — a mark beside your name: ${m.treatment}`,
      })),
    ],
  },
  {
