...
| 7 | **SMTP / email sender** (Resend, Postmark, SES) | Password reset and address confirmation are both built and both need a sender you own: set `EMAIL_DRIVER=resend` + `RESEND_API_KEY` (or `smtp` + `SMTP_URL`) and an `EMAIL_FROM` on your own domain. Until then the server refuses to start in production rather than printing reset links to a log. The flows are complete on `EMAIL_DRIVER=console` in development |
| 8 | **At least one ad network account** — start with BitLabs or Adsterra | Their signup is their KYC. You paste the callback secret |
| 9 | **Plan payment details** — bank/eSewa/Khalti/IME Pay to receive NPR | Platform revenue only: a store plan, annual rent, and a person's ByteBikri Plus month arrive on the same details and are matched by hand at your volume. Four variables, all optional: `PAY_ESEWA_ID`, `PAY_KHALTI_ID`, `PAY_IMEPAY_ID`, `PAY_BANK_ACCOUNT`. Until one is set the billing page says "not configured" and names the variable rather than showing a placeholder account |
