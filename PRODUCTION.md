...
| 7 | **SMTP / email sender** (Resend, Postmark, SES) | Password reset is built and needs a sender you own: set `EMAIL_DRIVER=resend` + `RESEND_API_KEY` (or `smtp` + `SMTP_URL`) and an `EMAIL_FROM` on your own domain. Until then the server refuses to start in production rather than printing links to a log. Address verification is not built yet |
